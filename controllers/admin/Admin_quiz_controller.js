...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new quizzesAdminEditViewModel(
      db.quiz,
      "Edit quiz",
      "",
      "",
...
app.get(
  ...
  async function (req, res, next) {
    try {
      ...
      var viewModel = new quizzesAdminDetailViewModel(
        db.quiz,
        "Quiz details",
        "",
        "",
...
app.get("/admin/api/configuration", async function (req, res, next) {
  const configurationModel = require("../../view_models/activities_admin_add_configuration_table");

  const viewModel = new configurationModel(
    db.quiz,
    "Add configurations",
    "",
    "",
  ...
  try {
    if (req.validationError) {
      viewModel.error = req.validationError;
      return res.render("admin/Edit_TerminateConfig", viewModel);
    }

    viewModel.session = req.session;

    const data = await db.quiz.insert({ p, h, count });
